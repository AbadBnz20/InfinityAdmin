...
import { DeleteUserCity } from "@/actions/usercity.action";

export const columns = [
  { name: "Nombre", uid: "name" },
  { name: "Region", uid: "state.name" },
...
export const TableUserCity = ({
  ...
  const renderCell = useCallback((item: UserCity, columnKey: React.Key) => {
    switch (columnKey) {
      case "name":
        return (
          <div>
      ...
      case "actions":
        return (
          <div className="flex items-center gap-4">
            ...
            {deletecell && (
              <ModalConfirm
                idItem={item.IdUserCity}
                Ondelete={DeleteUserCity}
              />
            )}
          </div>
        );
